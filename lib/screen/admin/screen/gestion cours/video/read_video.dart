...
class ReadVideos extends StatefulWidget {
  const ReadVideos({super.key});

  @override
  State<ReadVideos> createState() => _ReadVideosState();
}

class _ReadVideosState extends State<ReadVideos> {
  List<Videos> users = [];
  VideosController videosController = VideosController();
  List<Videos> _foundInfo = [];
  ...
  void _sortNew(String ascending) {
    ...
    setState(() {
      results = users
          .where((user) => user.number.toLowerCase().contains(ascending))
          .toList();
      log(results.map((e) => e.number).toString());
      setState(() {
        _foundInfo = results;
      });
    });
  ...
  getData() async {
    users = await videosController.getAllVideos();
    setState(() {
      isLoaded = false;
      _foundInfo = users;
    });
    ...
    return Scaffold(
        floatingActionButton: FloatingActionButton(
          onPressed: () {
           Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => const CreateVideos()),
            );
          },
        ...
        appBar: AppBar(
          ...
          title: const Text("Gestion de videos"),
        ),
                      ...
                      return ListTile(
                        ...
                        trailing: Row(
                          ...
                          children: [
                            ...
                            IconButton(
                                onPressed: () {
                                  videosController.deleteVideos(item.id);
                                  setState(() {
